"""Logika asisten percakapan BaliGuide: klasifikasi maksud (TF-IDF + Naive Bayes), slot preferensi, dan ranking destinasi."""
...
TRAINING_DATA_CORE = [
    ...
    ("pantai untuk foto aesthetic", "pantai"),
    ...
    ("mau ke beach", "pantai"),
    ("pantai tersembunyi", "pantai"),
    ("ingin berenang di laut", "pantai"),
    ...
    ("camping di bali", "alam"),
    ("mendaki gunung batur", "alam"),
    ("wisata sawah", "alam"),
    ("danau di bali", "alam"),
    ("mau ke air terjun", "alam"),
    ("sunrise di gunung", "alam"),
    ("petualangan alam bali", "alam"),
    ("river tubing", "alam"),
    ("taman nasional bali", "alam"),
    ("wisata outdoor", "alam"),
    ("healing di alam", "alam"),
    ("menikmati alam terbuka", "alam"),
    # --- BUDAYA ---
    ...
    ("tradisi bali", "budaya"),
    ("mau ke besakih", "budaya"),
    ("belajar budaya bali", "budaya"),
    ("wisata adat", "budaya"),
    ("seni budaya bali", "budaya"),
    ("arsitektur bali", "budaya"),
    ("pertunjukan seni", "budaya"),
    ("melukat di bali", "budaya"),
    # --- KULINER ---
    ("wisata kuliner", "kuliner"),
    ("mau cari makan", "kuliner"),
    ("cafe instagramable bali", "kuliner"),
    ("seafood bali", "kuliner"),
    ("kuliner lokal bali", "kuliner"),
    ("restoran romantis", "kuliner"),
    ("mau belanja oleh-oleh", "kuliner"),
    ("pasar tradisional bali", "kuliner"),
    ("makanan khas bali", "kuliner"),
    ("beli souvenir", "kuliner"),
    ("kopi bali", "kuliner"),
    ("warung makan bali", "kuliner"),
    # --- HIBURAN ---
    ...
    ("wahana bali", "hiburan"),
    ("rafting di bali", "hiburan"),
    ("atv bali", "hiburan"),
    ("mau ke waterbom", "hiburan"),
    ("gwk bali", "hiburan"),
    ("mau naik ayunan ekstrem", "hiburan"),
    ("seru-seruan di bali", "hiburan"),
    ("mau lihat lumba-lumba", "hiburan"),
    ("safari bali", "hiburan"),
    # --- MURAH / BUDGET ---
    ("wisata murah", "murah"),
    ...
    ("wisata gratis", "murah"),
    ("hemat di bali", "murah"),
    ("liburan low budget", "murah"),
    ("tiket murah bali", "murah"),
    ("tidak punya banyak uang", "murah"),
    ("cari yang ekonomis", "murah"),
    # --- TERBAIK ---
    ("tempat terbaik di bali", "terbaik"),
    ("wisata paling populer", "terbaik"),
    ("destinasi viral bali", "terbaik"),
    ("wajib dikunjungi", "terbaik"),
    ("tempat paling keren", "terbaik"),
    ("spot terfavorit", "terbaik"),
    ("bali paling terkenal", "terbaik"),
    # --- LOKASI ---
    ("wisata di badung", "lokasi_badung"),
    ("kuta bali", "lokasi_badung"),
    ("seminyak bali", "lokasi_badung"),
    ("uluwatu", "lokasi_badung"),
    ("wisata di ubud", "lokasi_gianyar"),
    ("gianyar", "lokasi_gianyar"),
    ("tegalalang", "lokasi_gianyar"),
    ("wisata di denpasar", "lokasi_denpasar"),
    ("sanur", "lokasi_denpasar"),
    ("wisata di karangasem", "lokasi_karangasem"),
    ("amed", "lokasi_karangasem"),
    ("wisata di buleleng", "lokasi_buleleng"),
    ("lovina", "lokasi_buleleng"),
    ("singaraja", "lokasi_buleleng"),
    ("wisata di tabanan", "lokasi_tabanan"),
    ("bedugul", "lokasi_tabanan"),
    ("wisata di bangli", "lokasi_bangli"),
    ("kintamani", "lokasi_bangli"),
    ("wisata di klungkung", "lokasi_klungkung"),
    ("nusa penida", "lokasi_klungkung"),
    ("wisata di jembrana", "lokasi_jembrana"),
    # --- LANJUT / LAINNYA ---
    ("ada lagi", "lanjut"),
    ("selain itu", "lanjut"),
    ("yang lain", "lanjut"),
    ("ada rekomendasi lain", "lanjut"),
    ("mau yang berbeda", "lanjut"),
    ("kasih alternatif lain", "lanjut"),
    ("opsi lain", "lanjut"),
    # --- TERIMA KASIH ---
    ...
    ("thanks", "terima_kasih"),
    ("thx", "terima_kasih"),
    ("oke bagus", "terima_kasih"),
    ("oke siap", "terima_kasih"),
    # --- BANTUAN / BINGUNG ---
    ("bingung mau ke mana", "bantuan"),
    ("tolong bantu", "bantuan"),
    ("rekomendasiin dong", "bantuan"),
    ("saya tidak tahu", "bantuan"),
    ("bantu saya pilih", "bantuan"),
    ("minta saran", "bantuan"),
    ("saran wisata bali", "bantuan"),
    # --- JAWABAN PROFIL (untuk slot filling) ---
    ("saya suka foto", "jawaban_profil"),
    ("saya senang hiking", "jawaban_profil"),
    ("budget saya 200rb", "jawaban_profil"),
    ("pergi berdua", "jawaban_profil"),
    ("bawa keluarga", "jawaban_profil"),
    ("mau yang tenang", "jawaban_profil"),
    ("saya suka petualangan", "jawaban_profil"),
    ("mood santai", "jawaban_profil"),
    ("saya dari badung", "jawaban_profil"),
    ("ingin ke ubud", "jawaban_profil"),
    ("mau yang sepi", "jawaban_profil"),
    ("mau yang ramai", "jawaban_profil"),
    ("pergi sama teman", "jawaban_profil"),
    ("honeymoon", "jawaban_profil"),
    ("solo traveling", "jawaban_profil"),
    ("tidak mau jauh-jauh", "jawaban_profil"),
    ("umur saya 25", "jawaban_profil"),
...
def infer_requested_category(teks: str) -> str | None:
    ...
    scores = {
        kat: sum(1 for kw in kws if kw in teks_norm)
...
SLOT_QUESTIONS = {
    ...
    "budget_level": [
        "Perkiraan budget per orang untuk tiket masuk:\n"
        "- Hemat (di bawah Rp 20.000)\n"
        "- Sedang (Rp 20.000 – Rp 100.000)\n"
        "- Premium (di atas Rp 100.000)",
        "Untuk budget tiket masuk, pilih hemat, sedang, atau premium?",
    ],
    "mood_preferences": [
        "Suasana seperti apa yang kamu inginkan?\n"
        "- Tenang & healing\n"
        "- Seru & ramai\n"
        "- Romantis\n"
        "- Petualangan\n"
        "- Keluarga (ramah anak)",
        "Pilih suasana: santai, seru, romantis, atau petualangan?",
    ],
    "hobbies": [
        "Aktivitas yang paling kamu minati:\n"
        "- Foto / konten\n"
        "- Hiking / trekking\n"
        "- Kuliner & belanja\n"
        ...
        "- Healing & meditasi\n"
        "- Petualangan ekstrem",
        "Saat liburan, aktivitas apa yang paling kamu suka?",
    ],
    "preferred_locations": [
        "Ada area favorit di Bali?\n"
        "Contoh: Ubud, Seminyak/Kuta, Nusa Penida, Buleleng, Karangasem.\n"
        'Jika tidak ada, tulis "bebas".',
        "Ingin fokus di kabupaten atau wilayah tertentu, atau bebas?",
    ],
}
...
def infer_place_profile(row):
    ...
    hobbies = [
        n for n, kws in PROFILE_HINTS["hobbies"].items() if any(k in teks for k in kws)
    ]
    moods = [
        n
        for n, kws in PROFILE_HINTS["mood_preferences"].items()
        if any(k in teks for k in kws)
    ]
    trip_types = [
        n
        for n, kws in PROFILE_HINTS["trip_type"].items()
        if any(k in teks for k in kws)
    ]
    mobility = (
        "aktif"
...
def score_row(
    row, preferences, kategori_target=None, lokasi_target=None, exclude_names=None
):
    ...
    teks = normalize_text(
        f"{row.get('deskripsi','')} {row.get('aktivitas','')} {row.get('nama_tempat','')}"
    )
    ...
    if kategori_target:
        kategori_map = {
            ...
            "alam": "alam",
            "budaya": "budaya",
            "kuliner": "kuliner & belanja",
            "hiburan": "taman hiburan",
        }
...
def get_ranked_recommendations(
    preferences, kategori_target=None, lokasi_target=None, exclude_names=None, top=4
):
    scored = []
...
def build_recommendation_response(judul, hasil, preferences, suffix=""):
    if not hasil:
        return (
            "Belum ada destinasi yang cocok dengan kriteria tersebut.\n\n"
    ...
    if preferences.get("trip_type"):
        summary.append(f"perjalanan {preferences['trip_type']}")
    if preferences.get("mood_preferences"):
        summary.append(f"suasana {', '.join(preferences['mood_preferences'][:2])}")
    if preferences.get("budget_level"):
        summary.append(f"budget {preferences['budget_level']}")
    if preferences.get("hobbies"):
        summary.append(f"hobi {', '.join(preferences['hobbies'][:2])}")
    if summary:
        lines.append(f"\n\n*Rekomendasi mempertimbangkan: {', '.join(summary)}.*")
    if suffix:
        lines.append(suffix)
    lines.append("\n\nAda pertanyaan lanjutan, atau ingin alternatif destinasi lain?")
    return "".join(lines)
...
def chat_ml(pesan_user: str, riwayat: list = None, preferences: dict = None) -> str:
    ...
    if intent == "terima_kasih":
        return random.choice(
            [
                "Sama-sama. Semoga liburannya menyenangkan.\n\nKalau butuh tempat lain atau ada pertanyaan, silakan tulis lagi.",
                "Senang bisa membantu. Selamat berlibur di Bali.\n\nJika ingin rekomendasi lain, kabari saja.",
            ]
        )
    ...
    if intent == "salam" and jumlah_pesan == 0:
        return (
            "Halo, selamat datang di <b>BaliGuide</b>.\n\n"
            "Saya membantu mencarikan destinasi wisata di Bali yang sesuai.\n\n"
            "Sebelum rekomendasi diberikan, boleh jawab beberapa pertanyaan singkat "
            "agar hasilnya lebih sesuai kebutuhanmu.\n\n"
    ...
    if sudah_ada_intent_wisata and confidence < 50 and jumlah_pesan <= 6:
        ...
        if pertanyaan:
            ...
            if intent in {"pantai", "alam", "budaya", "kuliner", "hiburan"}:
                labels = {
                    "pantai": "pantai",
                    "alam": "alam/pegunungan",
                    "budaya": "budaya",
                    "kuliner": "kuliner",
                    "hiburan": "hiburan",
                }
                ack = f"Baik, kamu tertarik wisata <b>{labels.get(intent,intent)}</b>.\n\n"
            elif lokasi:
                ack = f"Area <b>{lokasi}</b> ya, siap!\n\n"
            return (
                ack + "Sebelum saya rekomendasikan, boleh tanya dulu:\n\n" + pertanyaan
            )
    ...
    if not any(merged_prefs.values()):
        return (
            "Hai, saya BaliGuide.\n\n"
            "Agar rekomendasi lebih tepat, jawab dulu beberapa hal berikut.\n\n"
            + SLOT_QUESTIONS["trip_type"][0]
        )
...
def chat_response(
    pesan_user: str, riwayat: list = None, preferences: dict = None
) -> str:
