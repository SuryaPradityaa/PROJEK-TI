        ...
        for _, row in df.iterrows():
            insert_rows.append(
                (
                    ...
                    float(clean_value(row.get('latitude') or 0)) if clean_value(row.get('latitude')) is not None else None,
                    float(clean_value(row.get('longitude') or 0)) if clean_value(row.get('longitude')) is not None else None,
                )
            )
...
def get_wisata_rows(tempat="", kategori=""):
    conn = get_db()
    try:
...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "ganti-dengan-secret-key-acak-yang-panjang")
    ...
    try:
        ...
        try:
            with conn.cursor() as c:
                ...
                c.execute(
                    "SELECT id FROM users WHERE nama = %s",
                    (nama,)
                )
                if c.fetchone():
                ...
                c.execute(
                    "SELECT id FROM users WHERE gmail = %s",
                    (gmail,)
                )
                if c.fetchone():
    ...
    try:
        ...
        try:
            with conn.cursor() as c:
                ...
                c.execute(
                    "SELECT id, password, role FROM users WHERE nama = %s",
                    (nama,)
                )
                user = c.fetchone()
        finally:
...
def get_kategori():
    ...
    try:
        seed_wisata_records(conn)
        with conn.cursor() as c:
            c.execute("SELECT DISTINCT kategori FROM wisata WHERE kategori IS NOT NULL AND kategori <> '' ORDER BY kategori")
            rows = c.fetchall()
    finally:
        conn.close()
...
def get_rekomendasi():
    ...
    hasil = get_wisata_rows(tempat, kategori)
    if not hasil:
...
def admin_list_wisata():
    ...
    try:
        seed_wisata_records(conn)
        with conn.cursor() as c:
            c.execute("SELECT id, nama_tempat, kategori, kabupaten, rating, harga_tiket_wni, harga_tiket_wna, jam_buka, aktivitas, latitude, longitude FROM wisata ORDER BY id DESC")
            rows = c.fetchall()
    finally:
        conn.close()
    return jsonify(rows)
...
def tambah_suka():
    ...
    try:
        with conn.cursor() as c:
            c.execute("SELECT id FROM suka WHERE user_id = %s AND nama_tempat = %s", (user_id, nama_tempat))
            if c.fetchone():
                return jsonify({"message": "Sudah ada di favorit"})
            c.execute("INSERT INTO suka (user_id, nama_tempat) VALUES (%s, %s)", (user_id, nama_tempat))
            conn.commit()
    finally:
        conn.close()
...
def hapus_suka():
    ...
    try:
        with conn.cursor() as c:
            c.execute("DELETE FROM suka WHERE user_id = %s AND nama_tempat = %s", (user_id, nama_tempat))
            conn.commit()
    finally:
        conn.close()
...
def hapus_riwayat(riwayat_id):
    ...
    try:
        with conn.cursor() as c:
            c.execute("DELETE FROM riwayat_kunjungan WHERE id = %s AND user_id = %s", (riwayat_id, user_id))
            conn.commit()
    finally:
        conn.close()
...
def chatbot():
    ...
    try:
        ...
        if len(pesan) > 500:
            return jsonify({"balasan": "Pesan terlalu panjang (maks 500 karakter)."}), 400
...
def update_preferences():
    ...
    if not isinstance(hobbies, list) or not isinstance(mood_preferences, list) or not isinstance(preferred_locations, list):
